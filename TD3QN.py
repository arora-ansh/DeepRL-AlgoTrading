...
class D3QN(nn.Module):
    ...
    def __init__(self, numberOfInputs, numberOfOutputs, numberOfNeurons=numberOfNeurons, dropout=dropout, advantage = "maximum"):
        """
        ...
        self.advantage_1 = nn.Linear(numberOfNeurons, int(numberOfNeurons/2))
        self.advantage_f = nn.Linear(int(numberOfNeurons/2), numberOfOutputs)

        self.value_1 = nn.Linear(numberOfNeurons, int(numberOfNeurons/2))
        self.value_f = nn.Linear(int(numberOfNeurons/2), 1)
        ...
        torch.nn.init.xavier_uniform_(self.advantage_1.weight)
        torch.nn.init.xavier_uniform_(self.advantage_f.weight)
        
        torch.nn.init.xavier_uniform_(self.value_1.weight)
        torch.nn.init.xavier_uniform_(self.value_f.weight)
    ...
    def forward(self, input):
        ...
        advantage = F.leaky_relu(self.advantage_1(x))
        advantage = F.leaky_relu(self.advantage_f(advantage))
        value = F.leaky_relu(self.value_1(x))
        value = F.leaky_relu(self.value_f(value))

        if self.advantage == "average":
            baseline = torch.mean(advantage, dim=1, keepdim = True)
        if self.advantage == "maximum":
            baseline = torch.max(advantage, dim=1, keepdim = True)[0]

        output = value + (advantage - baseline)
        return output
...
class TD3QN:
    ...
    def __init__(self, observationSpace, actionSpace, numberOfNeurons=numberOfNeurons, dropout=dropout, 
        ...
        self.policyNetwork = D3QN(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
        self.targetNetwork = D3QN(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
